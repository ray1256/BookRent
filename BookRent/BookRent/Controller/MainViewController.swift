...
class MainViewController: UIViewController, LoginButtonDelegate {
    ...
    @IBAction func login(_ sender: Any) {
        Auth.auth().signIn(withEmail: account.textfield.text!, password: password.textfield.text!, completion: {
            ...
            /*
            if user != nil{
                /*let NickNameAlert = UIAlertController(title: "輸入綽號", message: "", preferredStyle: .alert)
                let NickNameAction = UIAlertAction(title: "Ok", style: .default){ (action) in
                    let NickName = NickNameAlert.textFields![0]
                    
                }
                NickNameAlert.addAction(NickNameAction)
        ...
        if let user = user{
            ...
            NickNameAlert.addAction(NickNameAction)
            NickNameAlert.addTextField{(NickName) in
            }
            ...
            updateRequest.displayName = NickNameAlert.textFields![0].text
